...
@section('content')
    <div class="hms-page-toolbar"><div><h2 class="hms-page-heading">{{ isset($role) ? 'Edit Role' : 'Create New Role' }}</h2></div>
        <a href="{{ route('admin.roles.index') }}" class="hms-back-btn">
    ...
    <div class="hms-panel hms-panel-padded">
        <form action="{{ route('admin.roles.store') }}" method="POST">
            ...
            <div class="mb-6">
                <label for="name" class="hms-label">Role Name:</label>
                <input type="text" id="name" name="name" class="hms-input focus:ring-2 focus:ring-blue-500" placeholder="Role Name" value="{{ old('name', $role->name ?? '') }}" required {{ (isset($role) && $role->name === 'Super Admin') ? 'readonly' : '' }}>
            </div>

            <h3 class="hms-section-title">Assign Permissions</h3>

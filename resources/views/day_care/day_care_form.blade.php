...
@section('content')
    <div class="hms-page-toolbar"><div><h2 class="hms-page-heading">Day Care Procedure</h2></div>
...
@if($errors->any())
    ...
    <div class="hms-panel hms-panel-padded mb-5">
        <form id="day_care_form" action="{{ route('day-care.store') }}" method="POST">
            ...
            <div class="hms-form-grid mb-6">
                ...
                <div>
                    <label for="hours_of_stay" class="hms-label">Hours of Stay:</label>
                    <input type="number" id="hours_of_stay" name="hours_of_stay" class="hms-input focus:ring-2 focus:ring-blue-500" min="0" value="0">
                </div>
            </div>
            ...
            <div class="mb-6">
                <label for="procedure_details" class="hms-label">Procedure Notes:</label>
                <textarea id="procedure_details" name="procedure_details" rows="3" class="hms-input focus:ring-2 focus:ring-blue-500" placeholder="Describe the day care procedure here..."></textarea>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Shortcut to open patient search modal
            ...
            patientSearchModal.addEventListener('keydown', function(event) {
                ...
                if (event.key === 'ArrowDown') {
                    ...
                    event.preventDefault();
                    const prevIndex = (activeIndex - 1 + results.length + results.length) % results.length;
                    results.forEach(el => el.classList.remove('bg-blue-100'));
            ...
            function calculateAllTotals() {
                ...
                const totalBill = totalConsultantFees;
            ...
            dayCareForm.addEventListener('submit', async function(event) {
                ...
                if (!mrNoInput.value || Object.keys(selectedConsultants).length === 0 || !amountPaidInput.value) {
                    alert('Please fill out all required fields: MR No, at least one Consultant, and Amount Paid.');
                    return;
                }
...
// --- Form Submission ---
            ...
            printSlipBtn.addEventListener('click', async function(event) {
                event.preventDefault();
                await saveForm(true);
            });
            ...
            function printReceipt() {
                if (Object.keys(selectedConsultants).length === 0) {
                    showStatusMessage('Please add at least one consultant before printing.', false);
                    return;
                }

    ...
    <div class="hms-panel hms-panel-padded mb-5">
        ...
        <form action="{{ isset($supplier) ? route('store.supplier.update', $supplier->id) : route('store.supplier.store') }}" method="POST">
            ...
            <div class="hms-form-grid mb-6">
                <div>
                    <label for="name" class="hms-label">Name:</label>
                    <input type="text" id="name" name="name" class="hms-input @error('name') border-red-500 @enderror" placeholder="e.g., Jane Doe" value="{{ old('name', $supplier->name ?? '') }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="company_name" name="company_name" class="hms-input @error('company_name') border-red-500 @enderror" placeholder="e.g., Pharma Inc." value="{{ old('company_name', $supplier->company_name ?? '') }}" required>
                    @error('company_name')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="contact_number" class="hms-label">Contact Number:</label>
                    <input type="tel" id="contact_number" name="contact_number" class="hms-input @error('contact_number') border-red-500 @enderror" placeholder="e.g., +923xx-xxxxxxx" value="{{ old('contact_number', $supplier->contact_number ?? '') }}">
                ...
                <div>
                    ...
                    <input type="text" id="address" name="address" class="hms-input @error('address') border-red-500 @enderror" placeholder="e.g., 123 Main St, Anytown" value="{{ old('address', $supplier->address ?? '') }}">
                    @error('address')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="reference" class="hms-label">Reference:</label>
                    <input type="text" id="reference" name="reference" class="hms-input @error('reference') border-red-500 @enderror" placeholder="e.g., Dr. Smith" value="{{ old('reference', $supplier->reference ?? '') }}">
                    @error('reference')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex items-center mt-6">
                    <label class="hms-checkbox-row">
                        <input type="checkbox" name="is_active" id="is_active" value="1" class="hms-checkbox" {{ old('is_active', $supplier->is_active ?? true) ? 'checked' : '' }}>

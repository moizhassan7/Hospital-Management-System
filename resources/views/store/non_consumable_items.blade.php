        ...
        <form action="{{ isset($nonConsumableItem) ? route('store.non_consumable_items.update', $nonConsumableItem->id) : route('store.non_consumable_items.store') }}" method="POST">
            ...
            <div class="hms-form-grid mb-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" class="hms-input @error('name') border-red-500 @enderror" placeholder="e.g., X-Ray Machine, Hospital Bed" value="{{ old('name', $nonConsumableItem->name ?? '') }}" required>
                    @error('name')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="asset_tag" class="hms-label">Asset Tag:</label>
                    <input type="text" id="asset_tag" name="asset_tag" class="hms-input @error('asset_tag') border-red-500 @enderror" placeholder="e.g., INV-XRAY-001" value="{{ old('asset_tag', $nonConsumableItem->asset_tag ?? '') }}">
                    @error('asset_tag')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="unit" class="hms-label">Unit (e.g., pcs):</label>
                    <input type="text" id="unit" name="unit" class="hms-input @error('unit') border-red-500 @enderror" placeholder="e.g., pcs, unit" value="{{ old('unit', $nonConsumableItem->unit ?? '') }}">
                    @error('unit')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" id="purchase_price" name="purchase_price" class="hms-input @error('purchase_price') border-red-500 @enderror" placeholder="e.g., 5000.00" min="0" step="0.01" value="{{ old('purchase_price', $nonConsumableItem->purchase_price ?? '') }}" required>
                ...
                <div>
                    <label for="company_name" class="hms-label">Manufacturer/Supplier:</label>
                    <input type="text" id="company_name" name="company_name" class="hms-input @error('company_name') border-red-500 @enderror" placeholder="e.g., Siemens, Philips" value="{{ old('company_name', $nonConsumableItem->company_name ?? '') }}">
                    @error('company_name')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="purchase_date" class="hms-label">Purchase Date:</label>
                    <input type="date" id="purchase_date" name="purchase_date" class="hms-input @error('purchase_date') border-red-500 @enderror" value="{{ old('purchase_date', (isset($nonConsumableItem) && $nonConsumableItem->purchase_date) ? $nonConsumableItem->purchase_date->format('Y-m-d') : date('Y-m-d')) }}" required>
                    @error('purchase_date')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="warranty_expiry_date" class="hms-label">Warranty Expiry Date:</label>
                    <input type="date" id="warranty_expiry_date" name="warranty_expiry_date" class="hms-input @error('warranty_expiry_date') border-red-500 @enderror" value="{{ old('warranty_expiry_date', (isset($nonConsumableItem) && $nonConsumableItem->warranty_expiry_date) ? $nonConsumableItem->warranty_expiry_date->format('Y-m-d') : '') }}">
                    @error('warranty_expiry_date')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="depreciation_rate" class="hms-label">Depreciation Rate (%/year):</label>
                    <input type="number" id="depreciation_rate" name="depreciation_rate" class="hms-input @error('depreciation_rate') border-red-500 @enderror" placeholder="e.g., 10.00" min="0" max="100" step="0.01" value="{{ old('depreciation_rate', $nonConsumableItem->depreciation_rate ?? '0.00') }}" required>
                    @error('depreciation_rate')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="current_status" class="hms-label">Current Status:</label>
                    <select id="current_status" name="current_status" class="hms-input @error('current_status') border-red-500 @enderror" required>
                ...
                <div>
                    ...
                    <input type="text" id="barcode" name="barcode" class="hms-input @error('barcode') border-red-500 @enderror" placeholder="e.g., 987654321098" value="{{ old('barcode', $nonConsumableItem->barcode ?? '') }}">
                    @error('barcode')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="col-span-1 md:col-span-2 lg:col-span-1 flex items-center mt-6">
                    <label class="hms-checkbox-row">
                        <input type="checkbox" name="is_active" id="is_active" value="1" class="hms-checkbox" {{ old('is_active', $nonConsumableItem->is_active ?? true) ? 'checked' : '' }}>
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
            ...
            <form id="categoryForm" action="#" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="category_name" name="name" class="hms-input" placeholder="e.g., Medical Equipment" required>
                </div>

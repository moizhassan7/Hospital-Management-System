    ...
    <div class="hms-panel hms-panel-padded mb-5">
        ...
        <form action="{{ isset($emergencyService) ? route('emergency.update_service', $emergencyService->id) : route('emergency.store_service') }}" method="POST">
            ...
            <div class="hms-form-grid-2 mb-6">
                <div>
                    <label for="service_name" class="hms-label">Service Name:</label>
                    <input type="text" id="service_name" name="service_name" class="hms-input" placeholder="e.g., Ambulance Fee" value="{{ old('service_name', $emergencyService->name ?? '') }}" required>
                </div>
                <div>
                    <label for="service_fee" class="hms-label">Service Fee (PKR):</label>
                    <input type="number" id="service_fee" name="service_fee" class="hms-input" placeholder="e.g., 500.00" value="{{ old('service_fee', $emergencyService->fee ?? '0.00') }}" min="0" step="0.01" required>
                </div>
            </div>
